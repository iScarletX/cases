(() => {
  ...
  function renderRoute(){
    ...
    if(portrait())group.style.backgroundImage=`url("${asset('route-background')}")`;
  ...
  function renderCharacters(){
    ...
    c.characters.forEach((person,index)=>{
      ...
      const ratio=c.assets[art].width/c.assets[art].height;
      front.style.setProperty('--card-ratio',ratio);
      front.style.setProperty('--mobile-card-width',(ratio>.65?280:210)+'px');
